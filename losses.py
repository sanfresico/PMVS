...
def calc_video_cos(video, gt_summ, keyframe_index_list, mask_video_summ=None, dataset=None):
    ...
    for i in range(batch_size):
        ...
        num_key_frame = sim_mat.shape[1]  # 使用 sim_mat 的列数
        match_mat = np.zeros((sim_mat.shape[0], num_key_frame), dtype=int)  # 动态调整 match_mat 的行数
        sorted_index = np.dstack(np.unravel_index(np.argsort(-sim_mat.ravel()), sim_mat.shape))[0]
        ...
        for j in range(sorted_index.shape[0]):
            m, n = sorted_index[j]
            if m >= sim_mat.shape[0] or n >= num_key_frame:
                continue
        ...
        if np.sum(match_mat) == 0:
            cos_sim = 0  # 防止除以 0 的错误
        else:
...
class InfoNCE(nn.Module):
    ...
    def forward(self, q, k, neg, device='cuda:0'):
        ...
        # 正样本与负样本相似度
        l_pos = (q @ k.T) / self.temperature  # [1, 1]
        l_neg = (q @ neg.T) / self.temperature  # [1, T]
...
class Dual_Contrastive_Loss(nn.Module):
    def __init__(self, args=None):
        ...
        self.NCE_video = InfoNCE()
        self.NCE_text = InfoNCE()
