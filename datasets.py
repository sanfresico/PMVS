...
class MSMODataset(object):
    def __init__(self, mode='train', args=None):
        ...
        self.video_dict = np.load('{}/{}/feature/video_resnet50_{}.npy'.format(args.data_root, args.dataset, mode),
                                  allow_pickle=True).item()
        self.text_dict = np.load('{}/{}/feature/text_roberta_{}.npy'.format(args.data_root, args.dataset, mode),
                                 allow_pickle=True).item()
        if args.dataset == 'Daily_Mail':
            self.video_summ_dict = np.load(
                '{}/{}/feature/video_summ_resnet50_{}.npy'.format(args.data_root, args.dataset, mode),
                allow_pickle=True).item()
        else:
...
class VideoSummDataset(object):
    def __init__(self, keys, args=None):
        self.keys = keys
        self.video_dict = h5py.File(
            '{}/{}/feature/eccv16_dataset_{}_google_pool5.h5'.format(args.data_root, args.dataset,
                                                                     args.dataset.lower()), 'r')
    ...
    def __getitem__(self, index):
        ...
        change_points = video_file['change_points'][...].astype(
            np.int32)  # [S, 2], S: number of segments, each row stores indices of a segment
        n_frames = video_file['n_frames'][...].astype(np.int32)  # [N], N: number of frames, N = T * 15
        n_frame_per_seg = video_file['n_frame_per_seg'][...].astype(
            np.int32)  # [S], indicates number of frames in each segment
