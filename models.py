...
class MultiHeadAttentionWithSlidingWindow(nn.Module):
    def __init__(self,
                 dims,
                 k_dims=None,
                 v_dims=None,
                 ...
                 bias=True,
                 window_size=10):
        super(MultiHeadAttentionWithSlidingWindow, self).__init__()
    ...
    def __repr__(self):
        return ('{}(q_dims={}, k_dims={}, v_dims={}, h_dims={}, o_dims={}, '
                'heads={}, p={}, bias={}, window_size={})'.format(self.__class__.__name__,
    ...
    def forward(self, q, k=None, v=None, mask=None):
        ...
        seq_len = q.size(-2)
        # 计算滑动窗口掩码
        mask = torch.zeros(1, 1, seq_len, dtype=torch.float32, device=q.device)
        ...
        if mask is not None:
            ...
            mask = mask.repeat(att.size(0), 1, 1)
...
class MultiWayTransformer(nn.Module):
    def __init__(self, num_hidden, dropout_attn=0.1):
        ...
        self.attn_fusion = MultiHeadAttentionWithSlidingWindow(num_hidden, p=dropout_attn)
...
class Model_MSMO(nn.Module):
    def __init__(self, args=None):
        ...
        self.multiway_list = nn.ModuleList(
            [MultiWayTransformer(num_hidden, dropout_attn=args.dropout_attn)] * args.num_layers)
    ...
    def select_contrastive_embedding(self, score, embedding, mask, label):
        ...
        for i in range(B):
            ...
            key_embedding_index_expand = ndimage.binary_dilation(label[i].cpu().detach().numpy(), iterations=4).astype(
                np.int32)
    ...
    def forward(self, **kwargs):
        ...
        key_video_list, nonkey_video_list = self.select_contrastive_embedding(pred_video, video, mask_video[:, 1:],
                                                                              video_label)
        key_text_list, nonkey_text_list = self.select_contrastive_embedding(pred_text, text, mask_text[:, 1:],
                                                                            text_label)
...
class Model_BLiSS(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        for i in range(B):
            ...
            mask_fused[i, 1:1 + N_video_valid, 1 + N_video:1 + N_video + N_text_valid] = video_to_text_mask_list[
                i]  # [N_video_valid, N_text_valid] not consider the [CLS] token
            mask_fused[i, 1 + N_video:1 + N_video + N_text_valid:, 1:1 + N_video_valid] = text_to_video_mask_list[
                i]  # [N_text-1, N_video-1] not consider the [CLS] token
            pos_embed_segment_video = video_to_text_mask_list[i].to(torch.float32) @ self.pos_embed_segment[0,
                                                                                     :N_text_valid,
                                                                                     :]  # [N_video_valid, C]
...
class Model_VideoSumm(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # N_video_valid: number of actual video frames for each data sample
        # N_text_valid: number of actual text frames for each data sample
        mask_fused = torch.zeros((B, N_video + N_text, N_video + N_text), dtype=torch.long).to(
            mask_video)  # [B, N_video+N_text, N_video+N_text]
        for i in range(B):
            mask_fused[i, :N_video, :N_video] = mask_video[i].view(1, N_video).expand(N_video, -1)  # [N_video, N_video]
            mask_fused[i, N_video:, N_video:] = mask_text[i].view(1, N_text).expand(N_text, -1)  # [N_text, N_text]
        ...
        # select contrastive pairs for the intra-sample constrastive loss
        key_video_list, nonkey_video_list = self.select_contrastive_embedding(pred_video_cls, video, mask_video[:, 1:],
                                                                              video_label)
        key_text_list, nonkey_text_list = self.select_contrastive_embedding(pred_text_cls, text, mask_text[:, 1:],
                                                                            text_label)
    ...
    def predict(self, **kwargs):
        pred_video_cls, pred_video_loc, pred_video_ctr, pred_text_cls, pred_text_loc, pred_text_ctr, contrastive_pairs = self(
            **kwargs)
